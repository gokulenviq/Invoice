...
const TableThree = () => {
  ...
  const handleDownloadPdf = async (invoice) => {
    const { invoiceNo, clientName, total,date ,website ,clientAddress ,status} = invoice;
    try {
      const htmlContent = ReactDOMServer.renderToString(<InvoicePdf invoiceNo={invoiceNo} website={website} clientName={clientName} status={status} clientAddress={clientAddress} date={date} total={total} />);
      console.log(invoiceNo); // Just for debugging
        ...
        <table className="w-full table-auto">
          <thead>
            <tr className="bg-gray-2 text-left dark:bg-meta-4">
              <th className="min-w-[220px] py-4 px-4 font-medium text-black dark:text-white xl:pl-11">S.No</th>
